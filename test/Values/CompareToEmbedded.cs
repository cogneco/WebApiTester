...
namespace Cogneco.WebApiTester.Test.Values
{
		...
		public class CompareToEmbedded
		{
			...
			{
				using (var fixture = await WebApiTester.AspNetCoreServer.Start<Mockup.Startup>())
					await fixture.Get("/api/values/5").ContentEqualsResource("/Values/GetResource5.txt");
			}
			[Fact]
			public async Tasks.Task PostCollection()
			{
				using (var fixture = await WebApiTester.AspNetCoreServer.Start<Mockup.Startup>()) {
					await fixture.Post("/api/values", "values6").HasContentType("text/plain", "utf-8").ContentEqualsResource("/Values/PostCollection6.txt");
					await fixture.Get("/api/values").ContentEqualsResource("/Values/GetCollection6.json");
				}
			}
		}
}
